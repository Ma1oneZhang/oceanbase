                                   ...
                                   ObIArray<ObTableSchema> &table_schemas)
{
  ...
  if (table_schemas.count() <= 0) {
    ...
    LOG_INFO("succed to create _sys_core_table, start create all schemas", "table count", table_schemas.count());

    if(OB_FAIL(ret)){}
    else if (OB_FAIL(parallel_create_schema(ddl_service, table_schemas))) {
      LOG_WARN("fail to create all schemas", K(ret));
    }
...
int ObBootstrap::parallel_create_schema(
      ObDDLService &ddl_service,
      ObIArray<ObTableSchema> &table_schemas)
{
  int ret = OB_SUCCESS;
        ...
        while (OB_SUCC(ret)) {
          if (OB_FAIL(batch_create_schema(ddl_service, table_schemas, begin, i + 1))) {
            ...
            if (retry_times <= MAX_RETRY_TIMES) {
              ...
              usleep(100 * 1000L); // 0.1s
            }
          } else {
            LOG_WARN("batch create schema succeed", "table count", i + 1 - begin);
  ...
  LOG_INFO("parallel create schema finish", K(ret), "table count", table_schemas.count(),
      "total_time_used", now - begin_time);

  // check if all threads succeed
    ...
    if (OB_FAIL(trans.start(&ddl_service.get_sql_proxy(),
      ...
      for (int64_t i = begin; OB_SUCC(ret) && i < end; ++i) {
        ...
        if(is_core_table(table.get_table_id())){
          if(i!= end-1) {
            if(!is_core_table(table_schemas.at(i+1).get_table_id())){
